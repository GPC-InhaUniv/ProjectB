...
public class Rader : MonoBehaviour
{
    ...
    public void RegisterRaderObject(GameObject o, Image i)
    {
        Image image = Instantiate(i);
        raderObjects.Add(new RaderObject() { Owner = o, Icon = image });
    }
    ...
    void DrawRaderDots()
    {
        foreach(RaderObject ro in raderObjects)
        {
            Vector3 raderPos = (ro.Owner.transform.position - playerPos.position);
            float distToObject = Vector3.Distance(playerPos.position, ro.Owner.transform.position) * mapScale;
            float deltay = Mathf.Atan2(raderPos.x, raderPos.z) * Mathf.Rad2Deg  - playerPos.eulerAngles.y;
            raderPos.x = distToObject * Mathf.Cos(deltay * Mathf.Deg2Rad) ;
            raderPos.z = distToObject * Mathf.Sin(deltay * Mathf.Deg2Rad);

            ro.Icon.transform.SetParent(this.transform);
            ro.Icon.transform.position = new Vector3(raderPos.x, raderPos.z, 0) + this.transform.position;
        }
    }
